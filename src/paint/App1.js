...
const AddNodeOnEdgeDrop = () => {
    ...
    const reactFlowGraph = useRef(null);
    ...
    const onConnectEnd = useCallback(
      ...
      (event) => {
        ...
        if (targetIsPane) {
          ...
          const { top, left } = reactFlowGraph.current.getBoundingClientRect();
  ...
  if(isNode(selectedObject)){
    selectedObject.className+= " selected-node-additional"
    }
  if(isEdge(selectedObject)){
      selectedObject.markerEnd = {
        type: MarkerType.ArrowClosed,
        width: 20,
        height: 20,
        color: 'aqua',
  ...
  setSelectedObject(selectedObject)
    ...
    const handleSelectionChange = ({ nodes, edges }) => {
      if (nodes.length === 1 && edges.length === 0) {
        setSelectedObject(nodes[0]);
      } else if (nodes.length === 0 && edges.length === 1) {
        setSelectedObject(edges[0]);
      } else {
    ...
    const onDrop = useCallback(
      (event) => {
        ...
        // 位置
        const reactFlowBounds = reactFlowGraph.current.getBoundingClientRect();
      ...
      <div className="wrapper" ref={reactFlowWrapper} style={{overflow:"hidden"}}>
        ...
        <div style={{ height: '100%', width: '100%', display:"flex",flexDirection:"column", padding:0, margin:0 }}>
          ...
          <div style={{ height: '90%', width: '100%', display:"flex",flexDirection:"row", margin:0 }} >
            ...
            {/*显示主体  */}
            <div ref={reactFlowGraph} className={'my_div'} style={{height:'100%',width:'75%',margin:0}}>
